					...
					<div class="widget-content">
						...
						<ul class="nav nav-tabs">
						  <li <c:if test="${activeTab=='adtab'}">class="active"</c:if>>
						    <a href="./listAdmServlet">管理员</a><!-- adtab  data-toggle="tab"-->
						  </li>
								...
								<div class="tab-pane <c:if test="${activeTab=='adtab'}">active</c:if>"   id="adtab">
										<table class="table table-striped table-bordered">
											...
											<tr>
												...
												<td>
													<a href='<c:url value="showAdminServlet?loginname=${admin.loginname }"></c:url>'>${admin.loginname }</a>
												</td>
												<td>
													${admin.name }
												</td>
												...
												<td>
													<a href='<c:url value="deleteAdminServlet?loginname=${admin.loginname }"></c:url>'><font color=red>删除</font></a> | 
													<a href='<c:url value="showAdminServlet?loginname=${admin.loginname }"></c:url>'><font color=red>查看/修改</font></a>
												</td>
											</tr>
											</c:forEach>							
											...
											<tr>
												...
												<c:if test="${requestScope.pageCount!=0&&requestScope.curPage!=1}">
													<a href='<c:url value="checkAdminServlet?curPage=1&loginname=${requestScope.loginname}&name=${requestScope.name}&password=${requestScope.password}"/>'>首页</a>
													<a href='<c:url value="checkAdminServlet?curPage=${requestScope.curPage-1}&loginname=${requestScope.loginname}&name=${requestScope.name}&password=${requestScope.password}"/>'>前一页</a>
												</c:if>
												...
												<c:if test="${requestScope.pageCount!=0&&requestScope.curPage!=requestScope.pageCount}">
													<a href='<c:url value="checkAdminServlet?curPage=${requestScope.curPage+1}&loginname=${requestScope.loginname}&name=${requestScope.name}&password=${requestScope.password}"/>'>下一页</a>
													<a href='<c:url value="checkAdminServlet?curPage=${requestScope.pageCount}&loginname=${requestScope.loginname}&name=${requestScope.name}&password=${requestScope.password}"/>'>尾页</a>
												</c:if>
