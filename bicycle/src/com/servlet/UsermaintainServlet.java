...
/**
 * Servlet implementation class UsermaintainServlet
 */
@WebServlet("/usermaintainServlet")

public class UsermaintainServlet extends HttpServlet {				//用户报修
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UsermaintainServlet() {
        super();
    }
		...
		try{
			if(person==null){
				...
				if(bikeDao.findBikeById(bikeId)==null){
					...
					request.getRequestDispatcher("/userHomeServlet").forward(request, response);		//跳转到用户维修界面（即刷新当前界面）
				}
					...
					Maintain maintain = new Maintain(0, userId, bikeId, 0, place, new Timestamp(System.currentTimeMillis()));
