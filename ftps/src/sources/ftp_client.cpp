...
void FTPClient::start(){
	...
	try{
		...
		_ctx = SSL_CTX_new(SSLv23_method());			/* Create new context */
		if ( _ctx == NULL )
		{
...
#ifdef USE_SSL
		request = FTPRequest("AUTH","TLS").getRequest();
		*control_socket<< request;
		*control_socket>> response;
		SSL* ssl = SSL_new(_ctx);						/* create new SSL connection state */
		SSL_set_fd(ssl, control_socket->fd());				/* attach the socket descriptor */
		if ( SSL_connect(ssl) == -1 )			/* perform the connection */
			ERR_print_errors_fp(stderr);
		control_socket->ssl(ssl);
...
#ifdef USE_SSL
		SSL* ssl = SSL_new(_ctx);						/* create new SSL connection state */
		SSL_set_fd(ssl, data_socket->fd());				/* attach the socket descriptor */
		if ( SSL_connect(ssl) == -1 )			/* perform the connection */
			ERR_print_errors_fp(stderr);
		data_socket->ssl(ssl);
#endif
